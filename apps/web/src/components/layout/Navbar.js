  ...
  return (
    <nav className="sticky top-0 z-50 border-b border-gray-200 dark:border-gray-800 bg-white/80 dark:bg-gray-950/80 backdrop-blur-sm">
      <div className="max-w-7xl mx-auto px-4 h-14 flex items-center justify-between">
        <Link href={user ? '/dashboard' : '/'} className="font-bold text-xl text-brand">
          CodeJudge
        </Link>

        <div className="flex items-center gap-4">
          {user?.role === 'ADMIN' && (
            <Link href="/admin" className="text-sm text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white">
              Admin
            </Link>
          )}
          ...
          {user ? (
            ...
            <Link
              href="/login"
              className="text-sm px-4 py-1.5 rounded-lg bg-brand text-white hover:bg-brand-dark transition-colors"
            >
              Sign In
            </Link>
          )}
